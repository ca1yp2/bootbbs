...
import net.ca1yps.bootbbs.service.BoardService;
...
public class BoardController {
    ...
    public BoardController(BoardService service, FileService fileService){
    ...
    //게시물 조회
    @GetMapping("/")
    public String list(@PageableDefault(size=10, sort="id", direction= Sort.Direction.DESC) Pageable pageable, @RequestParam(value="searchKey", required = false) String searchKey, @RequestParam(value="searchVal", required = false) String searchVal, Model model){
        Page<Board> lists;
  ...
  public String view(@ModelAttribute("id") long id, Model model) {
    ...
    if (board.isPresent()) {
      ...
      List<FileEntity> files = fileService.getFilesByBid(board.get().getId());
    ...
    //글 등록
    @PostMapping("/write")
    public String write(@ModelAttribute Board board, @RequestParam("fileKey") Long fileKey){
        Board saved = service.save(board);
        fileService.updateBidByTempkey(fileKey, saved.getId());
        return "redirect:/";
    }
